    ...
    if (isset($_POST['edit_btn'])){
        ...
        if ($email != $currentEmail){
            ...
            if ($checkCount > 0){
                ...
                $stmt = $conn->prepare("UPDATE research_secretary SET college = :college, RorE = :RorE, campus = :campus, fullname = :fullname, email = :email WHERE id = :id");
                $stmt->execute(array(
                    'college' => $college,
                    'campus' => $campus,
                    'RorE' => $RorE,
                ...
                if ($ihap > 0){
                    header("Location: ../../../../account controller/admin/secretary account/research.secretary.account.php?success=Chairperson account has been successfully updated!");
                }
            }
        }else{
            $stmt = $conn->prepare("UPDATE research_secretary SET college = :college, RorE = :RorE, campus = :campus, fullname = :fullname, email = :email WHERE id = :id");
            ...
            if ($ihap > 0){
                ...
                header("Location: ../../../../account controller/admin/secretary account/research.secretary.account.php?success=Nothing change!");
            }
        }
        // end
    }else{
