        ...
        if (in_array($file_actual, $allowed)){
            if ($file_error === 0){
                if ($file_size < 5242880){
                    ...
                    $checktitle = $conn->prepare("SELECT research FROM all_research_data WHERE research=:research");
                    ...
                    if ($count == 0){  # Change equal sign(==) to greater than sign(>) to execute the program else display research title is already exist!
                        ...
                        if ($diff > 0){
                            ...
                            for ($o = 0; $o < $countAuthor; $o++){
                                ...
                                // $sqls = $conn->prepare("SELECT * FROM research_secretary WHERE email = :email AND rank = :rank AND isDelete = :isDelete");
                                // $sqls->execute(['email' => $arrayEmail[$o], 'rank' => "Author", 'isDelete' => "not"]);
                                ...
                                // if ($sqlsCount == 0){
                                    ...
                                    if ($insertCount > 0){
                                        ...
                                        $subject = "Account Access";
                                        $body = "You can access now to JRMSU Research Development and Extension Portal using this email: " . $arrayEmail[$o] . " and password: " . $randomPassword . " given by the " . $added_by . " from " . $campus . " campus under the " . $college . " from " . $RorE . " as " . $rank . "\n\n" . "Click here to login: jrmsu-red-smp.great-site.net";
                                        ...
                                        // Insert Notification for Data
                                        $body = "You've successully added " . $authors . " as " . $RorE . " data in " . $campus . " campus college of " . $college;
                                        ...
                                        // Insert Notification for Account
                                        $accountBody = "You've successfully added " . $authors . " as author accounts";
                                        $account = $conn->prepare("INSERT INTO notification SET body = :body, date = :date");
                                        $account->execute(['body' => $accountBody, 'date' => $date]);
                                    }
                                // }else{
                    ...
                    if ($checker){
                        header("Location: ../../../../account controller/admin/my added data/research.my.data.php?success=$success");
                    }else{
                        header("Location: ../../../../account controller/admin/my added data/research.my.data.php?already=$error");
                    }

                    // if ($error){
                    //     echo "research title is already exist!";
                    // }else{
                    //     echo "next step againn";
                    //     echo $research;
                    // }
                    // echo "<br><b>" . $countEmail;
                }else{
