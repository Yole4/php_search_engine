  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="../../../../res api/users account/users/unit head/attributes/change password/index.php?id=author" method="post">
      ...
      <form action="../../../../res api/users account/users/unit head/author/editAuthor.php" method="POST">
        ...
        <div class="modal-body">
          ...
          <!--====================== TAMPILISAN =======================-->
            ...
            <div class="form-group">
                <label>Email</label>
                <input type="email" name="email" id="myEmail" class="form-control" placeholder="Email" required>
            </div>
  ...
  <div class="modal-dialog" role="document" style="width:400px">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <div class="card card-primary card-outline">
          <div class="card-body box-profile">
            <form action="../../../../res api/users account/users/unit head/attributes/change profile/index.php?check=author" method="POST" enctype="multipart/form-data">
              ...
              <div class="card-body table-responsive p-0" style="height: 500px;">
                <table id="my-table">
                  ...
                  <?php 
                    $RorE = $fetch['RorE'];
                    $campus = $fetch['campus'];
                    // $mysql = $conn->prepare("SELECT * FROM research_secretary WHERE campus = :campus AND RorE = :RorE AND added_by = :added_by AND rank = :rank");
                    // $mysql->execute(['campus' => $campus, 'RorE' => $RorE, 'added_by' => "Unit Head", 'rank' => "Author"]);
                    $mysql = $conn->prepare("SELECT * FROM research_secretary WHERE isDelete = :isDelete");
                    ...
                    while ($row = $mysql->fetch(PDO::FETCH_ASSOC)){ 
                      ...
                      if ($row['college'] != ""){
                        if ((($getRorE == $RorE) && ($getCampus == $campus) && ($getRank == "Author")) || (($getRorE == $RorE) && ($getCampus == $campus) && ($getRank == "Author"))){  $tester = $tester + 1; ?>
                          <tr class="hover">
...
<script>
    ...
    const added = row.querySelector('td:nth-child(7)').textContent.toLowerCase();
    const foundMatch = fullname.includes(searchString) || email.includes(searchString) || college.includes(searchString) || added.includes(searchString);
